ActiveAdmin.register Item, :as => 'Destinations'  do
menu :label => "Destination"
 # controller do
  ...
  form do |f|
      ...
      f.input :category, :collection => Category.where(:is_destination => true).map{ |category| [category.Name, category.id] },:prompt => true
      f.input :Name
      f.input :slug
      f.input :short_tag
      ...
      f.inputs "Images" do
       ...
       # f.input :is_destination, :as => :boolean, :required => true
     f.actions 
   end
...
controller do
    def scoped_collection
       super.where(:category_id=>Category.where(:is_destination => true))
